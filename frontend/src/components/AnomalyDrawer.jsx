...
export function AnomalyDrawer({ anomaly, onClose, onApply, applying }) {
  ...
  useEffect(() => {
    ...
    const action = anomaly?.actions?.[0]
    if (action) api.changePreview({ anomaly_id: anomaly.id, action_id: action.id }).then((preview) => { if (active) setRoutePreview(preview) }).catch(() => null)
    return () => { active = false }
  }, [anomaly?.id, anomaly?.actions])
