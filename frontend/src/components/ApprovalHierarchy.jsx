...
export function ApprovalHierarchy({ request, preview, compact = false, onOpenLedger, onSubmit, submitting = false }) {
  ...
  const statusClass = isPreview ? 'preview' : requestStatus === 'verified' ? 'success' : requestStatus === 'rolled_back' ? 'returned' : requestStatus === 'rejected' || requestStatus === 'stale' ? 'danger' : requestStatus === 'returned' ? 'returned' : active ? 'active' : 'neutral'
