...
export default function App() {
  ...
  useEffect(() => {
    try { window.localStorage.setItem('nexusai.sidebarCollapsed', String(sidebarCollapsed)) } catch { /* ignore */ }
  }, [sidebarCollapsed])
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main className="main-shell"><Topbar title={title} subtitle={subtitle} theme={theme} onToggleTheme={toggleTheme} onScan={runScan} scanning={scanning} onMenu={() => setSidebarOpen((open) => !open)} onTour={() => setTourOpen(true)} onBell={() => setBellOpen((open) => !open)} onNotifications={() => setNotificationOpen((open) => !open)} escalationCount={escalationCount} notificationCount={notifications.filter((item) => !item.read).length} principal={principal} onSignOut={handleSignOut} />{pulse && <div className="realtime-indicator"><Wifi size={13} />Mesh live · {pulse.active_findings} active signals</div>}<div className="global-approval-flow"><ApprovalHierarchy compact request={currentRequest} preview={fallbackWorkflowPreview} onOpenLedger={() => navigate('changes')} /></div>
