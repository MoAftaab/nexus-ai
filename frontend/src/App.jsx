...
export default function App() {
  ...
  const refreshFromEvent = useCallback(async (message) => {
    ...
    try {
      const [nextDashboard, nextAnomalies, nextDocuments, nextAlerts, nextOutcomes, nextGraph, nextChanges, nextNotifications] = await Promise.all([api.dashboard(), api.anomalies(), api.documents(), api.alerts(), api.outcomes(), api.graph(), principal ? api.changes().catch(() => ({ items: [] })) : Promise.resolve({ items: [] }), principal ? api.notifications().catch(() => ({ items: [] })) : Promise.resolve({ items: [] })])
      setDashboard(nextDashboard); setAnomalies(nextAnomalies.items); setDocumentData(nextDocuments); setAlerts(nextAlerts.items); setOutcomeData(nextOutcomes); setGraph(nextGraph); setChangeRequests(nextChanges.items || []); setNotifications(nextNotifications.items || []); if (principal) setWorkflowData(await api.workflowSummary().catch(() => null))
  ...
  const content = useMemo(() => {
    ...
    if (page === 'changes') return <ChangeControl principal={principal} focusRequestId={changeFocus} />
    if (page === 'archive') return <AuditArchive />
    if (page === 'policy') return <AccessPolicyConsole />
    return <CommandCenter {...props} dashboard={dashboard} workflow={workflowData} onScan={runScan} scanning={scanning} />
  }, [page, anomalies, graph, dashboard, reconciliation, documentData, agentData, alerts, outcomeData, workflowData, loadGraph, inspectDocument, scanning, principal, changeFocus])
