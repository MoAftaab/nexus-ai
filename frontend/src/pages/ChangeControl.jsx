...
export function ChangeControl({ principal, focusRequestId }) {
  ...
  const canDecide = selected?.can_decide ?? Boolean(selected?.active_step?.required_role === principal?.role && selected.requested_by !== principal?.user_id)
  const submit = async () => {
  ...
  const decide = async (decision) => {
    ...
    try { await api.decideChange(selected.request_id, decision, comment); setComment(''); await load() } catch (cause) { setError(cause.message) } finally { setBusy(false) }
  }
  const rollback = async () => {
    setBusy(true); setError('')
    try { await api.rollbackChange(selected.request_id, rollbackComment); setRollbackComment(''); await load() } catch (cause) { setError(cause.message) } finally { setBusy(false) }
  }
