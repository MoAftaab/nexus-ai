...
export function ChangeControl({ principal, focusRequestId, onWorkflowChanged }) {
  ...
  return <div className="page change-control-page">
    ...
    <section className="change-grid"><article className="card-surface change-list"><div className="section-title"><div><span className="eyebrow"><Filter size={14} /> Requests</span><h3>Current change ledger</h3></div><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All stages</option><option value="submitted">Submitted</option><option value="draft">Draft</option><option value="waiting_for_details">Waiting for requested details</option><option value="awaiting_lead">Waiting for Operations Lead</option><option value="awaiting_manager">Waiting for Operations Manager</option><option value="awaiting_quality_compliance">Waiting for Quality and Compliance</option><option value="awaiting_director">Waiting for Supply Chain Director</option><option value="approved">Approved</option><option value="rejected">Rejected</option><option value="returned">Returned for changes</option><option value="cancelled">Cancelled</option><option value="verified">Approved and verified</option><option value="rolled_back">Rolled back to original source data</option></select></div>{items.map((item) => <button className={`change-row ${selected?.request_id === item.request_id ? 'selected' : ''}`} key={item.request_id} onClick={() => void selectRequest(item.request_id)}><span className={`severity-mark ${item.severity}`} /><div><strong>{item.title}</strong><small>{item.request_id} · {item.site_id} · {statusLabel(item.status)}</small></div><b>{currency(item.impact_euros)}</b><ArrowRight size={15} /></button>)}{!items.length && <div className="empty-state"><GitPullRequest size={21} /><h3>No change requests in this view</h3><p>Preview a control from Risk Intelligence to start the governed path.</p></div>}</article>
