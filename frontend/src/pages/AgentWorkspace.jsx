...
export function AgentWorkspace({ agents, communication, onChatStream, onSelectAnomaly }) {
  ...
  return <div className="page agents-page">
    ...
    <section className="agent-workspace-layout"><aside className="agent-roster card-surface"><div className="section-title"><div><span className="eyebrow"><Bot size={14} /> The mesh</span><h3>Specialist agents</h3></div></div>{agents?.map((agent, index) => <div className={`roster-agent ${loading ? 'thinking' : ''}`} style={{ '--mesh-delay': `${index * 260}ms` }} key={agent.name}><span className={`agent-orb ${agent.color}`}><i /></span><div><strong>{agent.name}</strong><small>{agent.role}</small></div><span>{loading ? 'reasoning' : agent.state}</span></div>)}<div className="handoff-trace"><span>Recent handoffs</span>{communication?.map((event) => <div key={`${event.from}-${event.time}`}><i /><p><b>{event.from}</b> → <b>{event.to}</b><small>{event.message}</small></p><time>{event.time}</time></div>)}</div></aside>
      <article className="chat-shell card-surface"><div className="chat-header"><div><span className="eyebrow"><Sparkles size={14} /> Control Tower copilot</span><h3>Operations reasoning</h3></div><span className="chat-status"><i />Streaming</span></div><div className="message-list" ref={listRef}>{messages.map((message, index) => <div className={`message ${message.role}`} key={`${message.role}-${index}`}><span className="message-avatar">{message.role === 'assistant' ? <Sparkles size={15} /> : 'AM'}</span><div>{message.role === 'assistant' && message.content ? <div className="message-bubble"><Markdown text={message.content} onCite={(id) => onSelectAnomaly?.({ id })} /></div> :message.role === 'assistant' && message.source === 'streaming' ? <AgentThinking agents={agents} trace={message.trace} communication={communication} /> : <p>{message.content}</p>}{message.source && message.source !== 'streaming' && <small className="model-source">{message.source === 'openai' ? 'Five GPT-5.4 mini specialists + orchestrator' : 'Evidence mode — enable GPT-5.4 mini with a server-side key'}</small>}{message.trace?.length ? <details className="agent-trace"><summary>{message.trace.length} agent handoffs and Markdown context</summary>{message.trace.map((handoff) => <div key={`${handoff.agent}-${handoff.role}`}><b>{handoff.agent}</b><span>{handoff.role} · {handoff.status}</span><small>{handoff.detail}</small></div>)}</details> : null}{message.suggestions?.length ? <div className="suggestion-actions"><span className="suggestion-label">Ask a follow-up</span>{message.suggestions.map((suggestion) => <button key={suggestion} onClick={() => send(`Walk me through this control before I approve it: "${suggestion}". What are the steps, risks and verification?`)}>{suggestion}<ChevronRight size={13} /></button>)}</div> : null}</div></div>)}</div><div className="suggested-prompts">{prompts.map((prompt) => <button key={prompt} onClick={() => send(prompt)}>{prompt}</button>)}</div><form className="chat-input" onSubmit={(event) => { event.preventDefault(); send() }}><textarea value={input} onChange={(event) => setInput(event.target.value)} rows="1" placeholder="Ask Control Tower about the operation…" /><button className="send-button" disabled={!input.trim() || loading} aria-label="Send message"><ArrowUp size={18} /></button></form></article>
    </section>
