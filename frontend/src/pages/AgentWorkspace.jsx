...
export function AgentWorkspace({ agents, communication, architecture, onChatStream, onSelectAnomaly }) {
  const [messages, setMessages] = useState([{ role: 'assistant', content: 'I’m connected to the current operational evidence. Ask about the whole dataset, any anomaly, the evidence behind a finding, why it matters, or which human-approved control is safest.' }])
  ...
  const sourceLabel = architecture?.provider === 'codecraft'
    ? `${architecture.provider_label} · ${architecture.model} specialists + WALT Coordinator`
    : architecture?.provider === 'ollama'
      ? `Ollama · ${architecture.model} local specialists + WALT Coordinator`
      : architecture?.provider === 'openai'
        ? `Direct OpenAI · ${architecture.model} specialists + WALT Coordinator`
        : architecture?.enabled ? `${architecture.provider_label} · ${architecture.model} specialist mesh` : 'Grounded evidence mode · deterministic controls'
