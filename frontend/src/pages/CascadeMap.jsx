...
function WhatIfPanel({ anomaly, onClose }) {
  ...
  // Key the fetch on the anomaly ID, not its object identity: App refreshes
  // the anomalies array on every live event, and re-running Monte-Carlo each
  // time would reset the panel to a spinner mid-presentation.
  const anomalyId = anomaly.id
  const actionIds = anomaly.actions.map((action) => action.id).join(',')
  useEffect(() => {
    let cancelled = false
    setScenarios(null); setError('')
    Promise.all(actionIds.split(',').filter(Boolean).map((actionId) => api.whatif(anomalyId, actionId)))
...
export function CascadeMap({ anomalies, graph, onLoadGraph, onSelectAnomaly }) {
  ...
  // Generation counter: switching cascades mid-stream must orphan the old
  // stream's deltas instead of letting them contaminate the new selection.
  ...
  useEffect(() => {
    ...
    if (selected) onLoadGraph(selected)
    setShowWhatIf(false); setExplanation(null)
  }, [selected, onLoadGraph])
  ...
  const explain = useCallback(async () => {
    ...
    } catch (cause) { if (explainGeneration.current === generation) setExplanation({ text: `The narrator is unavailable: ${cause.message}`, streaming: false }) }
  }, [selected])
  const showGraph = Boolean(selected) && open.length > 0
      ...
      <article className="full-map card-surface"><div className="full-map-header"><div>{active && <span className={`severity-pill ${active.severity}`}>{active.severity}</span>}<h3>{active?.title || 'Select a cascade'}</h3></div>{active && <div className="impact-stack"><span>Modeled exposure</span><strong>{currency(active.impact)}</strong></div>}</div>{showGraph && graph?.simulation && <div className="simulation-ribbon"><span><b>{Math.round(graph.simulation.propagation_probability * 100)}%</b> propagation probability</span><span><b>{currency(graph.simulation.expected_impact)}</b> expected exposure</span><span><b>{currency(graph.simulation.p90_impact)}</b> P90 exposure</span><small>{graph.simulation.trials.toLocaleString()} Monte-Carlo trials</small><button className="text-button" onClick={explain} disabled={explanation?.streaming}><Sparkles size={13} />{explanation?.streaming ? 'Narrating…' : 'Explain this cascade'}</button></div>}
        ...
        <div className="map-with-panel">
          {showGraph
            ...
            : <div className="graph-empty">All cascades are contained — the dependency twin is quiet.</div>}
