...
export function RiskIntelligence({ anomalies, onSelectAnomaly }) {
  ...
  return <div className="page intelligence-page">
    ...
    <section className="risk-table card-surface"><div className="risk-table-head"><span>Finding</span><span>Systems</span><span>Time to impact</span><span>Exposure <ArrowDownUp size={13} /></span><span>Control</span></div><div className="risk-table-body">{visible.map((item, index) => <button className={`risk-row ${item.status === 'resolved' ? 'contained-row' : ''}`} key={item.id} onClick={() => onSelectAnomaly(item)}><span className="risk-title"><i>0{index + 1}</i><span className={`severity-mark ${item.status === 'resolved' ? 'resolved' : item.severity}`} /><span><strong>{item.title}</strong><small>{item.id} · {item.sku} · {item.zone}</small></span></span><span className="system-tags">{item.system.split(' · ').map((system) => <em key={system}>{system}</em>)}</span><span className="deadline">{item.status === 'resolved' ? 'Contained' : item.time_to_impact}</span><strong className="impact-number">{currency(item.impact)}</strong><span className="control-preview"><b>{item.status === 'resolved' ? '✓ resolved' : item.actions[0] ? `${item.actions[0].confidence}%` : '—'}</b><small>{item.actions[0]?.title || 'Escalate for manual review'}</small></span></button>)}</div>{visible.length === 0 && <div className="empty-state"><Filter size={24} />{filter === 'contained' ? <><h3>Nothing contained yet</h3><p>Approve every control on a finding and it moves here with its value.</p></> : <><h3>No open risks match this view</h3><p>The board is clear — change a filter, run a scan, or check Contained.</p></>}</div>}</section>
  </div>
}
