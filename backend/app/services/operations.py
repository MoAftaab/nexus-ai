...
class OperationsStore:
    ...
    def dashboard(self) -> dict[str, object]:
        ...
        return {
            ...
            "agents": [
                {"name": "Sentinel", "role": "Detection", "state": "watching", "signal": f"{len(open_items)} active findings", "color": "cyan"},
                {"name": "Correlator", "role": "Linking", "state": "reasoning", "signal": f"{sum(1 for item in open_items if item.cascade_nodes)} modeled cascades", "color": "violet"},
                {"name": "Cascade", "role": "Simulation", "state": "ready", "signal": f"{sum(len(item.cascade_edges) for item in open_items)} active graph edges", "color": "violet"},
                {"name": "Impact", "role": "Quantification", "state": "ready", "signal": f"€{exposure:,} modeled", "color": "peach"},
                {"name": "Fix", "role": "Resolution", "state": "waiting", "signal": f"{sum(len(item.actions) for item in open_items)} approvals queued", "color": "mint"},
            ],
        }
    ...
    def incident_report(self, anomaly_id: str) -> str | None:
        ...
        lines += ["", "---", "_Generated by NexusAI. All corrective actions were approved by a human operator; source-system corrections are recorded in the audit trail above._"]
        return "\n".join(lines)
    ...
    def outcomes(self) -> dict[str, object]:
        ...
        return {
            ...
            "roi": {
                ...
                "note": "Weekly manual reconciliation would surface these findings up to 7 days later; Nexus flags them within minutes of the data diverging.",
            },
        }
    ...
    def agent_architecture(self) -> dict[str, object]:
        return {"model": "gpt-5.4-mini", "orchestrator": "Nexus", "specialists": [
            {"name": "Sentinel", "responsibility": "Detects statistical and rule-based source anomalies", "input": "Generated source records + selected ML score"},
            {"name": "Correlator", "responsibility": "Connects cross-system records and retrieved evidence", "input": "Verified findings + Markdown retrieval"},
            {"name": "Cascade", "responsibility": "Models downstream dependencies, probabilities and deadlines", "input": "Dependency graph + operational timestamps"},
            {"name": "Impact", "responsibility": "Quantifies preventable euro exposure", "input": "Cascade conditions + cost models"},
            {"name": "Fix", "responsibility": "Designs a safe, human-approved control plan", "input": "Root-cause evidence + ownership constraints"},
        ], "handoff_policy": "Specialists cannot mutate source data. The orchestrator summarizes their evidence, and an operator applies any recommended control through the audit endpoint."}
